...
function addAuthHeaders(headers = {}) {
  console.log("Adding auth headers for user:", $state.snapshot(user));
  if (user.token) {
...
async function handle401Redirect(host) {
  const authURL =
  ...
  if (typeof window === "undefined") {
    ...
    console.log("Client side redirect to Auth service" + authURL);
