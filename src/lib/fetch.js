...
function normalizeRedirectUrl(redirectUrl) {
	if (redirectUrl && redirectUrl.startsWith('/') && !redirectUrl.startsWith('/' + basePath)) {
...
async function performFetch(api, options = {}) {
	...
	try {
		...
		if (handleRedirects && (response.status === 302 || response.status === 301)) {
			const redirectUrl = normalizeRedirectUrl(response.headers.get('Location'));
